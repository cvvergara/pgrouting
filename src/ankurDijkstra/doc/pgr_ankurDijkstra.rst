...
``pgr_ankurDijkstra`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: ankurDijkstra(Minimal Use)
...
.. code-block:: none
    ...
    pgr_ankurDijkstra(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_ankurDijkstra.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: ankurDijkstra(Complete signature)
...
.. code-block:: none
    ...
    pgr_ankurDijkstra(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
