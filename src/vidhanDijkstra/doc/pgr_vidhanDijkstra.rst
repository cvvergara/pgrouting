...
``pgr_vidhanDijkstra`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: vidhanDijkstra(Minimal Use)
...
.. code-block:: none
    ...
    pgr_vidhanDijkstra(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_vidhanDijkstra.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: vidhanDijkstra(Complete signature)
...
.. code-block:: none
    ...
    pgr_vidhanDijkstra(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
