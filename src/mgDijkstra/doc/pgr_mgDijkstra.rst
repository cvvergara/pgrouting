...
``pgr_mgDijkstra`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. code-block:: none
    ...
    pgr_mgDijkstra(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_mgDijkstra.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: mgDijkstra(Complete signature)
...
.. code-block:: none
    ...
    pgr_mgDijkstra(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
