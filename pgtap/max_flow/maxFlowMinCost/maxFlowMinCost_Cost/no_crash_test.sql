...
SELECT CASE WHEN is_version_2() OR NOT test_min_version('3.2.0')  THEN plan(1) ELSE plan(81) END;
...
BEGIN
    FOR i IN 0..array_length(params, 1) LOOP
        ...
        q1 := 'SELECT * FROM pgr_maxFlowMinCost_Cost (';
        ...
        ELSE
            RETURN query SELECT * FROM set_eq(q1, 'SELECT NULL::FLOAT');
        END IF;
    END LOOP;
...
BEGIN
  IF is_version_2() THEN
    RETURN QUERY
    SELECT skip(1, 'New Function on 3.0.0');
    RETURN;
  ELSIF  NOT test_min_version('3.2.0') THEN
    RETURN QUERY
